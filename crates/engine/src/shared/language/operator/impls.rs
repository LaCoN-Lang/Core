...
impl OperatorKind {
	pub fn match_operator(tail: &[u8]) -> OpMatch {
		...
		// ─────────────────────────────────────────────
		// Single match on first byte —
		// compiler emits one jump table covering
		// both ASCII and Unicode branches
		// ─────────────────────────────────────────────
		match c0 {
			// ── ASCII operators ──────────────────────
			b'+' => match c1 {
			...
			b'/' => match c1 {
				...
				Some(b'*') => OpMatch { token_kind: TokenKind::BlockComment, consume_count: 2 },
				Some(b'=') => OpMatch::op(OperatorKind::SlashEqual, 2),
				Some(b'|') if c2 == Some(b'\\') => OpMatch { token_kind: TokenKind::LineComment, consume_count: 3 },
				_          => OpMatch::op(OperatorKind::Slash, 1),
			},
			b'=' => match c1 {
				Some(b'=') => match c2 {
					Some(b'=') => OpMatch::op(OperatorKind::EqualEqualEqual, 3),
					Some(b'>') => OpMatch::op(OperatorKind::EqualEqualGreater, 3),
